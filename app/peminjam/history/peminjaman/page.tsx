...
export default async function RiwayatPeminjam() {
    ...
    const riwayat = await prisma.kegiatan.findMany({
        include : {alat:true},
        where : {id_user:session?.id_user, jenis_kegiatan:'Peminjaman'}
    })
                ...
                <Item variant="outline" className='gap-5' key={rwyt.id_kegiatan}>
                    ...
                    <ItemActions>
                        {rwyt.status === 'Decline' ? (
                            ...
                            <form action={`/peminjam/${rwyt.id_kegiatan}/batal/pinjam`} method="POST">
                                <Button variant="outline" size="sm">
                                    Batalkan
                                </Button>
                            </form>
                        ) : rwyt.status === 'Approved' ? (
                            <form action={`/peminjam/${rwyt.id_kegiatan}/kembali`} method="POST">
                                <Button variant="outline" size="sm">
                                    Ajukan Pengembalian
                                </Button>
                            </form>
